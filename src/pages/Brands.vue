<script setup>
import BrandsBody from '@/components/brands/BrandsBody.vue';
import PagesHeaderComponent from '@/components/PagesHeaderComponent.vue';


</script>

<template>
    <PagesHeaderComponent background="linear-gradient(to bottom right, #7c3aed, #8b5cf6, #c084fc)" label="Top Brands"
        description="Shop from your favorite brands">
        <template #icon>
            <svg data-prefix="fas" data-icon="tags" class="svg-inline--fa fa-tags" role="img" viewBox="0 0 576 512"
                aria-hidden="true">
                <path fill="currentColor"
                    d="M401.2 39.1L549.4 189.4c27.7 28.1 27.7 73.1 0 101.2L393 448.9c-9.3 9.4-24.5 9.5-33.9 .2s-9.5-24.5-.2-33.9L515.3 256.8c9.2-9.3 9.2-24.4 0-33.7L367 72.9c-9.3-9.4-9.2-24.6 .2-33.9s24.6-9.2 33.9 .2zM32.1 229.5L32.1 96c0-35.3 28.7-64 64-64l133.5 0c17 0 33.3 6.7 45.3 18.7l144 144c25 25 25 65.5 0 90.5L285.4 418.7c-25 25-65.5 25-90.5 0l-144-144c-12-12-18.7-28.3-18.7-45.3zm144-85.5a32 32 0 1 0 -64 0 32 32 0 1 0 64 0z">
                </path>
            </svg>
        </template>
    </PagesHeaderComponent>
    <BrandsBody />
</template>

<style scoped></style>