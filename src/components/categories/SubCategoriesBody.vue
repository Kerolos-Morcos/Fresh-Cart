...
defineProps(['customCategory']);

async function fetchSubCategories() {
    isLoading.value = true;
    const res = await fetchData({ url: `/v1/subcategories`, });
    if (res) {
        data.value = res.data;
    }
...
<template>
    <div class="container mx-auto px-4 py-10">
        <RouterLink
            class="inline-flex items-center gap-2 font-medium text-gray-600 hover:text-primary-600 transition-colors mb-6"
            to="/categories">
            <svg data-prefix="fas" data-icon="arrow-left" class="w-3 svg-inline--fa fa-arrow-left" role="img"
        ...
        <div class="mb-6">
            <h2 class="text-lg font-bold text-gray-900">{{ data?.length }} Subcategories in {{
                customCategory?.name }}
            </h2>
        </div>
            ...
            <RouterLink v-for="category in data" :key="category._id"
                ...
                <div
                    ...
                    <svg data-prefix="fas" data-icon="folder-open"
                        class="w-7 svg-inline--fa fa-folder-open text-2xl text-primary-600" role="img"
                ...
                <h3 class="font-bold text-gray-900 text-lg group-hover:text-primary-600 transition-colors mb-2">
                    {{ category.name }}
                </h3>
                <div
                    class="flex items-center gap-2 text-sm text-primary-600 opacity-0 group-hover:opacity-100 transition-opacity">
                    <span>Browse Products</span>
                    <svg data-prefix="fas" data-icon="arrow-right" class="w-3 svg-inline--fa fa-arrow-right text-xs"
