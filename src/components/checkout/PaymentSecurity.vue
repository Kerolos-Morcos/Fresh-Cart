<script setup>

</script>

<template>
    <div class="flex items-center gap-3 p-4 bg-linear-to-r rounded-xl border border-green-100 mt-4"
        style="--from: var(--color-green-50); --to: var(--color-emerald-50);">
        <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
            <svg data-prefix="fas" data-icon="shield-halved" class="w-4 svg-inline--fa fa-shield-halved text-green-600"
                role="img" viewBox="0 0 512 512" aria-hidden="true">
                <path fill="currentColor"
                    d="M256 0c4.6 0 9.2 1 13.4 2.9L457.8 82.8c22 9.3 38.4 31 38.3 57.2-.5 99.2-41.3 280.7-213.6 363.2-16.7 8-36.1 8-52.8 0-172.4-82.5-213.1-264-213.6-363.2-.1-26.2 16.3-47.9 38.3-57.2L242.7 2.9C246.9 1 251.4 0 256 0zm0 66.8l0 378.1c138-66.8 175.1-214.8 176-303.4l-176-74.6 0 0z">
                </path>
            </svg>
        </div>
        <div>
            <p class="text-sm font-medium text-green-800">Secure & Encrypted</p>
            <p class="text-xs text-green-600 mt-0.5 font-medium">
                Your payment info is protected with 256-bit SSL encryption
            </p>
        </div>
    </div>
</template>

<style scoped></style>