...
async function addProductToCart() {
    ...
    if (result?.success) {
        ...
        if (authStore.isLoggedUser) {
            toastMessage('Successfully added to cart!', 'success');
        }
    } else {
...
<template>
    <button @click="addProductToCart" :disabled="isLoading"
        class="cursor-pointer disabled:cursor-not-allowed h-10 w-10 rounded-full flex items-center justify-center transition-all duration-300 text-white disabled:opacity-70"
        :class="success ? 'bg-green-500' : 'bg-primary-600 hover:bg-primary-700'">
        <svg v-if="success" data-prefix="fas" data-icon="check" class="w-4" viewBox="0 0 448 512">
