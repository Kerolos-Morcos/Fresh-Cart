...
async function addProductToCart() {
    ...
    if (authStore.isLoggedUser) {
        ...
        if (result?.cartData) {
            cartStore.cartData = result.cartData.data;
            cartStore.numOfCartItems = result.cartData.numOfCartItems;
            cartStore.totalCartPrice = result.cartData?.totalCartPrice;
        }
    }
    else {
        result = addToCartGuest(product, showSuccess);
        if (result?.cartData) {
            cartStore.cartData = result.cartData;
            cartStore.numOfCartItems = result.cartData.length;
        }
    }
...
<template>
    <button @click="addProductToCart" :disabled="isLoading"
        ...
        <LoadingSpinner v-if="isLoading" class="w-4" />
